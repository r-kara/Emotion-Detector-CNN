...
import sys
sys.path.append('/Users/kara/Desktop/github_projects/Emotion-Detector-CNN')
...
# Define the path to the Gender dataset
data_path = '../Part_3/Gender'
...
if __name__ == '__main__':
    # Load the model
    model = MainCNN()
    model.load_state_dict(torch.load('Models/gender_final_model_maincnn.pth')) ##../Part_2/Models/final_model_maincnn.pth or Models/gender_final_model_maincnn.pth

    # Define dictionaries to store evaluation results for each age group
    gender_groups = ['Female', 'Male']
    evaluation_results = {age_group: {} for age_group in gender_groups}
    ...
    for gender_group in gender_groups:
        # Create dataset and data loaders for the current gender group
        test_dataset = ImageFolder(root=f'{data_path}/{gender_group}/test', transform=transform)
        ...
        evaluation_results[gender_group]['Accuracy'] = accuracy
        evaluation_results[gender_group]['Precision'] = precision
        evaluation_results[gender_group]['Recall'] = recall
        evaluation_results[gender_group]['F1_Score'] = f1_score
    ...
    # Print results
    print("Macro-averaged Performance Metrics for Each Gender Group:")
    for gender_group, results in evaluation_results.items():
        print(f"\nGender Group: {gender_group}")
